...
def save_image(im, path, dims=None):
    ...
    if dims:
        ...
        im = resized.convert('RGB')
        im.save(path)
    else:
        im = im.convert('RGB')
        im.save(path, save_all=True)
...
def save_class_activation_images(org_img, activation_map, out_dir, file_name, hmp_dims):
    ...
    if (-1 in activation_map):
        ...
        path_to_file = os.path.join(out_dir, file_name+'_org.pdf')
        ...
        #save_image(heatmap, path_to_file)
        # Save heatmap on iamge
        path_to_file = os.path.join(out_dir, file_name+'_hmp.pdf')
